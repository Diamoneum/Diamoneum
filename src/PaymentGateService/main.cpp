...
void __stdcall serviceMain(DWORD dwArgc, char **lpszArgv)
{
    ...
    if (serviceStatusHandle == NULL) {
        logRef(Logging::FATAL, Logging::BRIGHT_RED)
            << "Couldn't make RegisterServiceCtrlHandler call: "
    ...
    if (SetServiceStatus(serviceStatusHandle, &serviceStatus) != TRUE) {
        logRef(Logging::FATAL, Logging::BRIGHT_RED)
            << "Couldn't make SetServiceStatus call: "
            << GetLastErrorMessage(GetLastError());
        return;
    }
...
int registerService()
{
    ...
    for (;;) {
        ...
        logRef(Logging::INFO)
            << "Please make sure "
            << moduleDir + "payment_service.conf"
            << " exists";
        break;
    }
...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        Logging::LoggerRef(pg.getLogger(), "main")(Logging::INFO)
            << "walletd v"

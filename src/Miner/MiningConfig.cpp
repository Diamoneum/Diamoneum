    ...
    : help(false)
{
    cmdOptions.add_options()
        (
            ...
            "address",
            po::value<std::string>(),
            "Valid cryptonote miner's address"
        )(
            "daemon-host",
            po::value<std::string>()->default_value(DEFAULT_DAEMON_HOST),
            "Daemon host"
        )(
            "daemon-rpc-port",
            po::value<uint16_t>()->default_value(static_cast<uint16_t>(RPC_DEFAULT_PORT)),
            "Daemon's RPC port"
        )(
            "daemon-address",
            po::value<std::string>(),
            "Daemon host:port. If you use this option you must not use "
            "--daemon-host and --daemon-port options"
        )(
            "threads",
            po::value<size_t>()->default_value(CONCURRENCY_LEVEL),
            "Mining threads count. Must not be greater than you concurrency level. "
            "Default value is your hardware concurrency level"
        )(
            "scan-time",
            po::value<size_t>()->default_value(DEFAULT_SCANT_PERIOD),
            "Blockchain polling interval (seconds). "
            "How often miner will check blockchain for updates"
        )(
            "log-level",
            ...
            "Mine exact quantity of blocks. 0 means no limit"
        )(
            ...
            po::value<int64_t>()->default_value(0),
            "Timestamp step for each subsequent block. "
            "May be set only if --first-block-timestamp has been set. "
...
void MiningConfig::parse(int argc, char **argv)
{
    ...
    if (!options["daemon-address"].empty()) {
        if (!options["daemon-host"].defaulted() || !options["daemon-rpc-port"].defaulted()) {
            throw std::runtime_error(
                "Either --daemon-host or --daemon-rpc-port is already specified. "
                "You must not specify --daemon-address"
            );
        }
    ...
    if (!options["block-timestamp-interval"].defaulted()
        && options["first-block-timestamp"].defaulted()) {

...
void adjustMergeMiningTag(Block& blockTemplate)
{
    if (blockTemplate.majorVersion == BLOCK_MAJOR_VERSION_2
        || blockTemplate.majorVersion >= BLOCK_MAJOR_VERSION_3) {
            ...
            if (submitBlock(m_minedBlock, m_config.daemonHost, m_config.daemonPort)) {
                ...
                if (m_config.blocksLimit != 0 && ++blocksMined == m_config.blocksLimit) {
                    m_logger(Logging::INFO)
                        << "Miner mined requested "
                        << m_config.blocksLimit
                        << " blocks. Quitting";
                    return;
                }
            }
    ...
    uint16_t daemonPort)
{
    try {
        ...
        m_logger(Logging::INFO)
            << "Block has been successfully submitted. Block hash: "
            << Common::podToHex(get_block_hash(minedBlock));

        return true;
    } catch (std::exception &e) {
        m_logger(Logging::WARNING)
            << "Couldn't submit block: " << Common::podToHex(get_block_hash(minedBlock))
            << ", reason: " << e.what();
        return false;
    }
}
    ...
    const std::string &miningAddress)
{
    try {
        ...
        m_logger(Logging::DEBUGGING)
            << "Requested block template with previous block hash: "
